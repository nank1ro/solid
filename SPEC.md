...
- **Transformation vs verbatim copy.** Solid reads every `.dart` file under `source/`. A file is transformed if it contains at least one `@Solid*` annotation (`@SolidState`, `@SolidEffect`, `@SolidQuery`, `@SolidEnvironment`), OR if a class it declares reaches a `@SolidState`-bearing class declared in another source file purely through a plain constructor-injected field or instance field (a "pure consumer") — in that narrower case only the `.value` reads reaching the injected class are lowered; the consuming class gains no `Disposable`/`dispose()` synthesis, since it owns no reactive member of its own to dispose. Otherwise the file is copied verbatim to the mirrored path under `lib/`. Non-`.dart` files (assets, configs, etc.) are always copied verbatim. The key is annotation presence or a resolvable cross-file `@SolidState` reference, not file extension. A `StatelessWidget`/`State<X>` reached the same way is a pure consumer too: its `build()` method gets the same `.value` lowering plus a `SignalBuilder` wrap around the tracked subtree — the same wrap-placement rule (Section 7) an `@SolidEnvironment`-consuming widget's `build()` already gets — with no widget-class Stateless→Stateful lift, since a plain constructor-injected field needs no `BuildContext`. Static-field-mediated DI (`static final AuthRepository instance = …;`, consumed elsewhere as `Holder.instance.session`) is recognized as well: a class reached only through a static field or singleton holder enters the registry the same as one reached through an instance field. When a file contains BOTH a pure-consumer widget and a pure-consumer plain class (e.g. a `StatelessWidget` alongside a router-guard class that reads a static-holder receiver like `Holder.instance.session`), both are lowered correctly against the same fully-resolved read of the file — the widget's `.value`-plus-`SignalBuilder` treatment and the plain class's `.value` lowering do not interfere with each other, including when the plain class's reactive read is a shape (a static-holder receiver, a for-in loop variable, `.first`) that only resolves through the type-resolved analyzer pass. If a pure-consumer widget's `SignalBuilder` wrap is the file's only reference to `flutter_solidart`, and the file already imports `flutter_solidart` with a `show`/`hide` combinator that doesn't expose `SignalBuilder` (e.g. a pre-existing `show Signal` for an unrelated hand-rolled signal), the generator repairs that import's combinators in place rather than leaving `SignalBuilder` unresolvable. A pure consumer whose ONLY link to the injected class is a bare-shorthand `super.x` constructor parameter (no type written at that position) is recognized too: detecting a pure consumer at all requires a syntactic (unresolved) pre-parse, and a bare `super.x`'s type is not present in the source text at that position — but the class's `extends` clause names the superclass, and the superclass declaration (found locally, or via the same import walk this pre-parse already does — same-package imports only, alias-aware: an unprefixed `extends Base` skips any `import ... as x;` directive, a prefixed `extends x.Base` considers only the import carrying that exact prefix) still carries the type syntactically, either on the matching super-constructor parameter or, for the common `Base(this.x)` shape, on the base class's own same-named instance field. The generator locates the super-constructor the subclass's initializer list targets (the unnamed constructor by default, or a named one via `super.named(...)`) and matches the bare parameter against it — positionally among the constructor's other `super.` parameters, or by name — falling back to the field declaration when the matched parameter is itself an untyped `this.x`. When the matched parameter is ITSELF a bare `super.x` (a chain of plain relaying subclasses, e.g. `GrandChild(super.x) extends Child(super.x) extends Base(this.x)`), the search recurses up the chain, up to five superclasses deep, before giving up. When that match can't be pinned down confidently, every candidate type name from the located superclass (every constructor parameter, every instance field) is seeded instead of guessing wrong or skipping silently — over-seeding a name is harmless, since the registry only fills from classes that actually carry `@SolidState` members. Locating the injected class's OWN declaration, once its name is known, follows the SAME file the superclass search already reached, plus exactly one more hop through that file's own imports (never a general transitive walk) — enough to cover the common shape where a consumer imports only its immediate superclass's file, and that file in turn imports the class the superclass's constructor names. A class reachable only through a longer chain of imports beyond that one extra hop is a documented residual, not recognized.
...
**Auto-dispose injection.** When a `Provider(...)` / `Provider<T>(...)` / `.environment<T>(...)` call site omits the `dispose:` named argument, the generator decides whether to insert `dispose: (context, provider) => provider.dispose()` per a four-tier rule (full detail in §4.9 rule 7):

1. `T` provably has a `dispose()` method (its own declaration, or inherited — e.g. `implements Disposable`, or `extends ChangeNotifier`, including transitively through a same-file base-class chain) → inject.
2. `T` is `@Solid*`-annotated — every Solid-lowered class always synthesizes `dispose()` in its `lib/` output, even before that synthesis has happened to the pre-lowering declaration the generator can see — recognized same-file OR cross-file → inject.
3. `T`'s declaration IS visible (resolved, or found in the file being scanned) and shows neither of the above → skip; the call site is left byte-identical, same as an explicit `dispose: null`.
4. `T`'s declaration is NOT visible anywhere the generator looked (typically a cross-file, non-`@Solid*` type reached on the pass that has no resolver available at all) → inject anyway. This is the deliberately conservative default: a type the generator truly cannot see is treated the same as the tool's original, pre-type-aware behavior, so a wrong guess fails LOUDLY at compile time (an `undefined_method` error on `.dispose()`) rather than silently leaking a resource. `dispose: null` is the explicit, always-available opt-out for this case.

Tier 2's cross-file recognition is name-based (by simple class name, not library identity) — see the caveat in §4.9 rule 7. The user writes the create-side only:
...
For a type with no `dispose()` method at all, the user writes no stub and no `dispose:` — the generator detects the absence and injects nothing, PROVIDED its declaration is visible to whichever check is running (tier 3 above):
...
lowers byte-identical (no `dispose:` argument added) when `AuthRepository` is declared in the same file as this call site, or — on the builder's no-annotation fast path only — anywhere the type resolves.

If instead `AuthRepository` were declared in ANOTHER file and this call site's file ALSO has at least one `@Solid*`-annotated class (forcing the generator's main lowering pass, which runs with no resolver — see §4.9 rule 7), the generator has no way to confirm `AuthRepository` is dispose-less: it falls to tier 4 and injects `dispose:` anyway. This is the one case where auto-injection can require the explicit opt-out below even for a genuinely plain type — accepted as the honest cost of "unknown → inject, loud failure over silent leak."

To opt out of auto-injection for a type that DOES have `dispose()` (e.g. to wire a non-default cleanup method, or because the instance must outlive this `Provider`), or for a plain cross-file type the generator couldn't prove dispose-less (the tier-4 case above), supply `dispose:` explicitly — any value, including `dispose: null`, suppresses the injection:
...
7. **Auto-dispose injection is type-aware.** When a `Provider<T>(...)`, `Provider(...)` (with inferred `T`), or `.environment<T>(...)` call site omits the `dispose:` named argument, the generator decides whether to insert `dispose: (context, provider) => provider.dispose()` per a four-clause decision table. `T` is read from the call site's explicit type argument when present, else inferred from the `create` callback's returned constructor call. `MultiProvider(...)` itself never receives a `dispose:` argument; the generator descends into its `providers:` list and applies the per-entry rule to each inner `Provider(...)`. Other arguments (`create:`, `child:`, `lazy:`, etc.) pass through byte-identical. `Provider<T>.value(...)` is not rewritten — it owns no instance and takes no `dispose:`.

   | # | Condition | Result |
   |---|---|---|
   | 1 | `T` provably has `dispose()` — own declaration, or inherited (e.g. `implements Disposable`, or a known disposable base like `ChangeNotifier`/`ValueNotifier`, including transitively through a same-file base-class chain: `class Mid extends ChangeNotifier {}` then `class Leaf extends Mid {}` in the same file still counts). | Inject. |
   | 2 | `T` is `@Solid*`-annotated (same-file OR cross-file). Every Solid-lowered class synthesizes `dispose()` in its `lib/` output — Section 10 — even though the checker usually can only see the PRE-lowering declaration, which has no `dispose()` yet. | Inject. |
   | 3 | `T`'s declaration IS visible to the check that's running (resolved, or found in the file being scanned) and shows neither 1 nor 2. | Skip — the call site is left byte-identical, same outcome as an explicit `dispose: null`. |
   | 4 | `T`'s declaration is NOT visible anywhere the running check looked. | Inject anyway — the deliberately conservative default, preserving the tool's original, pre-type-aware behavior for anything it truly can't see: a wrong guess fails LOUDLY at compile time (`undefined_method` on `.dispose()`) rather than silently leaking a resource. `dispose: null` is the explicit, always-available opt-out. |

   Two implementation paths exist and each reaches these clauses differently:
   ...
   **Footnote — generic call sites.** A `.environment<T>()` call where `T` is itself a class-level type parameter (e.g. `.environment<T>(() => makeT())` inside a method of `class Foo<T>`) can never resolve to a concrete declaration — there is no class named `T` for the registry or resolver to find — so it always falls through to clause 4 (inject-by-default). If the bound on `T` doesn't guarantee a `dispose()` method, the injected `provider.dispose()` call is a compile error. Such generic wrappers need an explicit `dispose:` argument (even `dispose: null`) at the call site; the generator cannot infer one safely.
...
Practical consequence: a user who declares `void dispose() {}` on a source class can pass that class to `Provider<Counter>(...)` or `.environment<Counter>(...)` without writing `dispose:` themselves — the generator detects the source-side `dispose()` declaration and injects `dispose: (context, provider) => provider.dispose()` automatically (see §4.9 rule 7). The source-side `void dispose() {}` stub is therefore doubly required: it is both what the generator's type-aware check looks for AND what makes `provider.dispose()` resolve without an unresolved-method error at the source layer. A class with no `dispose()` stub gets nothing injected, PROVIDED its declaration is visible to whichever check runs at that call site (§4.9 rule 7's clause 3) — no crash, no manual `dispose: null` needed. A plain, stub-less class declared in ANOTHER file, provided from a file that also has a `@Solid*`-annotated class, is the one case that still needs an explicit `dispose: null` even though it has no `dispose()` — the generator's main lowering pass has no resolver and cannot prove such a type plain (§4.9 rule 7's clause 4). The empty-body convention is fine — the dispose-body merge rule below prepends the synthesized reactive disposals to whatever the user wrote:
