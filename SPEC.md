...
- **Transformation vs verbatim copy.** Solid reads every `.dart` file under `source/`. A file is transformed if it contains at least one `@Solid*` annotation (`@SolidState`, `@SolidEffect`, `@SolidQuery`, `@SolidEnvironment`), OR if a class it declares reaches a `@SolidState`-bearing class declared in another source file purely through a plain constructor-injected field or instance field (a "pure consumer") — in that narrower case only the `.value` reads reaching the injected class are lowered; the consuming class gains no `Disposable`/`dispose()` synthesis, since it owns no reactive member of its own to dispose. Otherwise the file is copied verbatim to the mirrored path under `lib/`. Non-`.dart` files (assets, configs, etc.) are always copied verbatim. The key is annotation presence or a resolvable cross-file `@SolidState` reference, not file extension. Two shapes are named, residual gaps in the pure-consumer rule: a `StatelessWidget`/`State<X>` reached the same way is out of scope — its `build()` method would need SignalBuilder-wrap placement to rebuild reactively, a separate, larger change — and static-field-mediated DI (`static final AuthRepository instance = …;`, consumed elsewhere as `Holder.instance.session`) is not recognized either, because the cross-file seeding walk deliberately skips `static` fields, so a class reached only through a static field or singleton holder never enters the registry and its reads stay un-lowered.
