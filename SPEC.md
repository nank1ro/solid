...
- **Transformation vs verbatim copy.** Solid reads every `.dart` file under `source/`. A file is *transformed* when any of the following holds: (a) it contains at least one `@Solid*` annotation (`@SolidState`, `@SolidEffect`, `@SolidQuery`, `@SolidEnvironment`); (b) it contains a `Provider(...)` / `Provider<T>(...)` / `.environment<T>(...)` call site (Section 4.9 item 7 — auto-dispose injection); or (c) it imports `package:flutter_solidart/...` or `package:solid_annotations/...` (Section 4.10 — call-site `const` promotion). Otherwise the file is copied verbatim to the mirrored path under `lib/`. Non-`.dart` files (assets, configs, etc.) are always copied verbatim. The key is whether the file participates in any Solid-aware pass, not file extension alone.
...
### 4.10 Auto-`const` at call sites

Independent of class lowering, the generator inserts the `const` keyword before every constructor invocation in the file that the Dart language would consider const-promotable. The pass runs after Section 4.9 item 7's auto-dispose injection so any closure argument the dispose pass injects is part of the const-evaluability check.

**Eligibility.** A constructor call `Foo(...)` (or `Foo.named(...)`) is promoted to `const Foo(...)` iff:

1. The constructor is const. Resolved analysis is used to determine this: any class whose `ConstructorElement.isConst` is true qualifies — Flutter widgets (`MaterialApp`, `Scaffold`, `Text`, …), Solid-lowered widgets (`CounterPage` from another file), plain user classes declaring `const Foo(...)`, and types from any third-party package. The names of Solid-generated classes whose lowered constructor declaration gains `const` per Section 14 item 7 are also included, even when the source ctor lacked `const` (resolved analysis of the *source* AST would not see them otherwise).
2. Every argument is const-evaluable: literals (bool, int, double, null, simple string, symbol); `AdjacentStrings` of simple-string parts; already-`const` `InstanceCreationExpression`s; and bare constructor calls that are themselves const-promotable by rule 1. Explicit `new Foo(...)`, `StringInterpolation` (`'$x'`), list / map / set literals, identifier reads, and method calls are rejected.
3. The call is at the **outermost** const-eligible position in its expression tree. Once the outer call becomes a const constructor invocation, Dart's const-context elision makes the explicit `const` redundant on every nested const-eligible call (and would trip `unnecessary_const`), so the pass short-circuits at every promoted node — nested children are not visited.
4. The call does not already carry an explicit `const` or `new` keyword. Already-`const` calls are left alone; explicit-`new` calls are respected as the user's intent to allocate a fresh instance.

**Scope.** The pass runs over every source file the builder visits — files with `@Solid*` annotations, files with `Provider(...)` / `.environment<T>(...)` call sites, and files that merely import `package:flutter_solidart/...` or `package:solid_annotations/...` (per Section 2). It does NOT run on files copied verbatim — those have no Solid involvement and the lint stays the user's concern.

**Failure mode.** If type resolution for the file fails (missing transitive dependency, parse error in an imported file), the resolved-derived name set is empty for that file. Per-file declaration-emitted names (Solid-generated ctors) still drive promotion of those call sites; Flutter / third-party ctor promotions are skipped silently. No build failure; the user's `prefer_const_constructors` lint simply fires on the un-promoted sites until resolution is restored.

**Relationship to Section 14 item 7.** Section 14 item 7 governs the `const` keyword on the *constructor declaration* of a generator-rewritten widget. This section governs the `const` keyword on every *call site* of any const-constructible class anywhere in the output. The two passes are complementary: declaration-`const` is what makes a generated widget const-eligible at all; call-site-`const` is what keeps `prefer_const_constructors` silent at every place that widget (or any other const-eligible type) is constructed.
