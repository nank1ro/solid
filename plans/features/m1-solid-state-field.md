...
- **Imports.** Per SPEC Section 9, the generator adds `flutter_solidart` whenever the lowered code references one of its names. M1-era goldens kept `solid_annotations` verbatim because the rewriter preserved every source import; M8-01 later updated the contract so `solid_annotations` is pruned from the output unless the lowered code references the `Disposable` marker or the `.environment<T>()` extension. M8-02 added alphabetical ordering within each import group at emit time.
