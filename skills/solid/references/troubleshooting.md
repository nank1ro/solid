...
| `RangeError: Maximum call stack size exceeded` / cyclic reaction when an effect writes a signal | A signal write inside a `@SolidEffect` subscribes the effect to what it writes. Collection signals (`MapSignal`/`ListSignal`/`SetSignal`) read their value internally to diff on every element-write, so the write notifies → the effect re-runs → it writes again → forever. (Scalar writes don't self-subscribe.) | Read dependencies into locals first, then wrap only the write in the `untracked(() => …)` function form: `final c = counter; untracked(() => history = [...history, c]);`. Do NOT wrap the whole body — that untracks the dependency reads too, so the effect never re-runs. See [patterns.md §8](./patterns.md#8-writing-a-signal-inside-an-effect--untracked). |
...
| `undefined_method` on `.dispose()` inside a generated `.environment(...)`/`Provider(...)` dispose callback in `lib/` | `dispose:` was omitted and the created type's declaration isn't visible to the generator (cross-file, non-`@Solid*` type), so a dispose call was injected as the safe default — and the type has no `dispose()`. | Pass `dispose: null` explicitly to opt out, or give the type a `dispose()`. See [annotation-contract.md § Disposal](./annotation-contract.md#disposal). |
