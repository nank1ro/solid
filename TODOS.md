...
**Status:** DONE — new public function `validateSolidEnvironmentSameClassProvideAndConsume(unit)` added at the bottom of `packages/solid_generator/lib/src/target_validator.dart` and wired in `packages/solid_generator/lib/builder.dart` right after `validateSolidEnvironmentTargets`. The validator iterates `ClassDeclaration`s directly (the existing `_walkUnit` is per-member and can't see the build body alongside the env fields), collects each class's `@SolidEnvironment` field types into a `Map<typeText, fieldName>`, then walks the `build` method body via a private `_ProvideConsumeDetector extends RecursiveAstVisitor<void>` that records the first matching `Provider<T>(...)` constructor call OR `.environment<T>(...)` extension call whose explicit `<T>` lexeme is keyed in the map. First-match-wins (the rejection-test harness asserts `result.errors.length == 1`). Detection is unresolved-AST only — both fixtures use explicit type arguments. Two AST-shape gotchas baked into the visitor: (a) `Provider<T>(...)` without `new`/`const` parses as `MethodInvocation` with `target == null`, mirroring the dual-shape pattern in `placement_visitor.dart`'s `SignalBuilder` check — both branches share a `_tryRecord` helper; (b) `.environment<T>(...)` is an extension call whose target is ALWAYS non-null (opposite invariant from the `SignalBuilder` check). The error message names the consumer field, the consumed type, the specific offending construct (`Provider<Counter>(...)` vs `.environment<Counter>(...)`), and includes both SPEC §3.6 SwiftUI-idiomatic resolutions ("move the consumer to a child widget, or own the instance locally with `@SolidState`"). The inferred-T `.environment(...)` form is documented as an explicit scope limit (no test fixture exercises it; would require resolved AST). All 142 prior generator tests remain green.
