...
### TODO M7-02 — Publish solid_annotations and solid_generator at 2.0.0

**Goal:** Prepare both workspace packages for pub.dev v2 publication. Bump `packages/solid_annotations/pubspec.yaml` and `packages/solid_generator/pubspec.yaml` from `2.0.0-dev.1` to a stable `2.0.0`. Remove the development-gate `publish_to: 'none'` from `solid_generator/pubspec.yaml`. Replace `solid_generator`'s path dep on `solid_annotations` with the version constraint `^2.0.0`. Add `issue_tracker: https://github.com/nank1ro/solid/issues` to both pubspecs and a generator-specific `topics:` block (`code-generation`, `build-runner`, `framework`, `fine-grained-reactivity`, `swiftui`) to `solid_generator`. Add `## 2.0.0` blocks to both CHANGELOGs in Conventional Commits style (`**FEAT**`, `**FIX**`, `**CHORE**`); existing v1 entries preserved below. CHANGELOGs are user-facing — no SPEC references, no milestone tags, no internal section numbers; only entries describing genuinely new v2 deltas (the underlying source-to-lib transformer model and the four annotation classes existed in v1 already, so they do not appear as CHANGELOG entries). Local validation via `dart pub publish --dry-run` for both packages; the actual `dart pub publish` run is out of scope for this PR (the user runs it manually after merge: annotations first, then generator).

This re-uses the M7-02 slot. The earlier candidate "M7-02 = docs CI workflow" listed in `plans/features/m7-ci.md` shifts to M7-03 / M7-04 if it lands later.

**SPEC references:** Section 13 (operational concerns — release prep is the natural follow-up to the CI workflow shipped in M7-01).

**Files to modify:**

- `packages/solid_annotations/pubspec.yaml` — bump version, add `issue_tracker` (kept the existing `documentation:` field on user request)
- `packages/solid_generator/pubspec.yaml` — bump version, remove `publish_to: 'none'`, add `issue_tracker`, add `topics`, swap path dep for `^2.0.0`
- `packages/solid_annotations/CHANGELOG.md` — prepend `## 2.0.0` block
- `packages/solid_generator/CHANGELOG.md` — prepend `## 2.0.0` block
- `plans/features/m7-ci.md` — renumber candidate follow-ups (docs CI → M7-03, contributor infra → M7-04) and add M7-02 to the TODO sequence

**Files to create:**

- `packages/solid_generator/.pubignore` — exclude `test/` from the published archive (the golden-output files under `test/golden/outputs/` import `package:flutter/...` and `package:flutter_solidart/...` because they are generator-output snapshots; shipping them triggers 142 import-validation warnings on `pub publish` since `solid_generator` itself does not depend on flutter or flutter_solidart). Tests are dev-only; downstream pub.dev users do not need them.
- `packages/solid_generator/lib/solid_generator.dart` — thin library stub (no exports, just a `library;` doc comment) added to satisfy pub.dev's "expose a library matching the package name" convention. The build_runner entry point stays at `lib/builder.dart` (referenced from `build.yaml`); renaming it would break the builder.

**Acceptance:** Both pubspecs at `version: 2.0.0`. `publish_to: 'none'` absent from `solid_generator/pubspec.yaml`. `solid_generator` depends on `solid_annotations: ^2.0.0` (no path). Both CHANGELOGs have a `## 2.0.0` block summarizing v2 with the existing v1 entries preserved below. Both packages pass `dart pub publish --dry-run` locally with zero blocking warnings. `flutter pub get` at the repo root still resolves cleanly. The six M6 verification commands (CI gate) all pass locally. README install command (`flutter pub add solid_annotations flutter_solidart provider` + `dart pub global activate solid_generator`) needs no edit and continues to work once 2.0.0 is live on pub.dev.

**Dependencies:** M6-10, M7-01.
...
**Status:** DONE — `RewriteResult` extended with `bool emitsDisposable` (only `plain_class_rewriter.rewritePlainClass` ever sets it true; `stateless_rewriter`, `state_class_rewriter`, and `builder._renderOutput`'s passthrough branch all set it false). `computeOutputImports` gained a `required bool referencesSolidAnnotations` parameter; when false it filters URIs starting with `package:solid_annotations/` from the input list. The doc-comment "left to `dart fix --apply`; this rewriter never drops a source import" deleted and replaced with the new SPEC §9 prune rule. `builder._renderOutput` restructured to assemble the body text first, compute `referencesSolidAnnotations = results.any((r) => r.emitsDisposable) || RegExp(r'\.environment\b').hasMatch(body)`, then call `computeOutputImports`. The `\b` after `\.environment` matches both `.environment(` and `.environment<` calls — the documented acceptable false-positive (a user method literally named `environment` keeps the import live) is preserved per SPEC §9 rationale. New unit-test file `packages/solid_generator/test/import_rewriter_test.dart` adds three focused tests on `computeOutputImports`: drop when `referencesSolidAnnotations: false`, keep when true, and preserve every non-`solid_annotations` URI while still appending `flutter_solidart` / `provider` per their flags. New golden fixture `m8_01_environment_extension_used` covers the `.environment<T>()` textual-scan keep path end-to-end (a `@SolidState`-bearing `StatelessWidget` whose Stateless→Stateful split runs (so `emitsDisposable` is FALSE) but whose lowered `_AppState.build` returns `const Placeholder().environment<Counter>(...)` — `solid_annotations` survives only because of the regex hit). 39 existing goldens lost their `solid_annotations` import; 11 keep it because they emit `Disposable` (`m1_06`, `m1_08`, `m4_08_effect_on_plain_class`, `m5_09_query_on_plain_class`, all five `m6_02_*` variants, `m6_04_cross_class_value_read`, `m6_06_multi_environment`); plus the new `m8_01_environment_extension_used` keeps it via the `.environment` scan — total 12 keep-list files. `example/analysis_options.yaml` lost the `unused_import: ignore` line and its three-line explanatory comment block; `example/lib/counter.dart` had its now-unused `package:solid_annotations/solid_annotations.dart` import removed by hand (the curated `example/lib/` outputs carry post-`dart fix` `const`-on-constructor and explicit-type forms that the generator doesn't emit until M8-03 — running `build_runner` end-to-end would strip those and break `flutter test example/`, so a hand edit on the import line keeps the example consistent until M8-03 closes the gap). `example/lib/m6_09_environment_app.dart` already kept `solid_annotations` because its lowered `class Counter implements Disposable` triggers the structural keep path, so no edit there. Verification: `dart format --set-exit-if-changed .` reports 191 files / 0 changed; `dart analyze --fatal-infos` from the workspace root, `dart analyze packages/solid_generator/test/golden/outputs/`, and `dart analyze packages/solid_annotations` all report zero issues; `dart test packages/solid_generator/ -r github --fail-fast` passes 145 tests (existing 141 + 3 import-rewriter unit tests + 1 new golden); `flutter test example/ -r github` passes 9 tests including the M6-09 `const CounterDisplay().environment<Counter>(...)` pump-and-dispose scenarios.
...
**Status:** DONE — `computeOutputImports` gained a final `result.sort(_compareImportUris)` call after the existing keep/drop and append logic, with a 7-line private comparator that ranks URIs by group (`dart:` = 0, `package:` = 1, relative = 2) and falls back to `String.compareTo` on the full URI within each group. Full-URI compare reproduces the analyzer's `directives_ordering` lint exactly: `package:flutter/material.dart` sorts before `package:flutter_solidart/flutter_solidart.dart` because `/` (0x2F) < `_` (0x5F) in ASCII. Sort runs after the appends so `flutter_solidart` / `provider` land in their alphabetical slot rather than at the tail. The function's doc-comment was rewritten to describe the new ordering and the appended-URI placement (the previous "preserved verbatim and in original order" wording is gone). 12 goldens regenerated — exactly the M8-01 keep-list (`m1_06`, `m1_08`, `m4_08_effect_on_plain_class`, `m5_09_query_on_plain_class`, all five `m6_02_*` variants, `m6_04_cross_class_value_read`, `m6_06_multi_environment`, `m8_01_environment_extension_used`); each diff is a single-block reorder that moves `solid_annotations` from the top (or middle, on `m6_02_user_dispose_body`) of the `package:` group to its alphabetical slot. All other goldens already matched the new sort and produced no diff. No relative imports exist in the corpus. `import_rewriter_test.dart` gained a new `group('computeOutputImports alphabetical sort (SPEC §9)', ...)` with five tests: `dart:` before `package:` regardless of source order, alphabetical within `package:` group, slash-vs-underscore tie, relative imports placed last, and appended URIs landing in alpha slot rather than at the tail; the existing third test was renamed from "preserves … and appends …" to "preserves … and sorts appended … into alpha position" with a clarifying comment. `example/analysis_options.yaml` lost lines 7–10 (the `directives_ordering: ignore` line plus its three-line explanatory comment block). Two hand-edits were needed to keep the example package analysis-clean now that the suppression is gone: `example/lib/m6_09_environment_app.dart` had its imports alphabetized (`flutter/material`, `flutter_solidart`, `provider`, `solid_annotations`) — same hand-curate-until-M8-03 pattern M8-01 used on `example/lib/counter.dart`; and `example/source/counter.dart` had its two imports swapped (`flutter/material` before `solid_annotations`) so the build_runner input also passes the lint. Verification: `dart format --set-exit-if-changed .` reports 191 files / 0 changed; `dart analyze --fatal-infos` from the workspace root, `dart analyze packages/solid_generator/test/golden/outputs/`, and `dart analyze example/` all report zero issues; `dart test packages/solid_generator/ -r github --fail-fast` passes 150 tests (M8-01's 145 + 5 new sort tests); `flutter test example/ -r github` passes the 9 widget/dispose tests; both `dart pub publish --dry-run` runs are clean modulo the expected mid-PR "checked-in files are modified in git" warning that resolves on commit.
...
**Status:** DONE — `_emitCtors` in `packages/solid_generator/lib/src/stateless_rewriter.dart` gained a third parameter `bool classFieldsAreConstSafe` and prefixes `const ` on each ctor whose `_isConstEligibleCtor` check passes when the flag is true. Three new private helpers carry the logic: `_isConstEligibleCtor` (per-ctor checks — factory/external/already-const filters, every formal is `FieldFormalParameter` or `SuperFormalParameter`, body is `EmptyFunctionBody` or empty `BlockFunctionBody`, init list is either empty or every entry is a `ConstructorFieldInitializer` with a literal RHS); `_isLiteralRhs` (returns true for `BooleanLiteral` / `DoubleLiteral` / `IntegerLiteral` / `NullLiteral` / `SimpleStringLiteral` / `SymbolLiteral` — explicitly excludes `StringInterpolation`, `AdjacentStrings`, identifier reads, instance creation, and operator expressions); and `_classFieldsAreConstSafe` (the class-level gate — walks retained widget-bound non-`@SolidState` instance fields and returns false if any is neither final nor const, with static fields skipped). The literal-RHS scope for clause (c) is the smallest rule that covers all three SPEC test scenarios PLUS the `m1_13_multiple_constructors` named-ctor case `Counter.named({super.key}) : title = 'Named'` — without it m1_13's named ctor would round-trip non-const yet still trip `prefer_const_constructors_in_immutables` (the literal `'Named'` IS const-evaluable). Identifier-RHS and `const C(...)` RHS are out of scope, a tractable future extension. Four new focused goldens land in `test/golden/inputs/` + `outputs/`: `m8_03_const_ctor_super_key` (canonical `Widget({super.key})` → `const`), `m8_03_const_ctor_field_formal` (`{super.key, required this.label}` with `final String label` → `const`), `m8_03_const_ctor_literal_initializer` (`Widget.named({super.key}) : label = 'Named'` → `const`, validates the literal-RHS branch — the spec asked for "at least three" focused goldens; the fourth is justified because the literal-RHS path is a distinct branch from empty-init-list and is required for m1_13 to lint clean), and `m8_03_const_ctor_assert_preserved` (`{super.key, required this.label}) : assert(label.isNotEmpty, '...')` → preserved verbatim — the assert deliberately calls a non-const getter so analyzer's `canBeConst` returns false and the lint stays silent on the verbatim output; a naïve `: assert(x > 0)` would still trip the lint because asserts ARE allowed in const ctors when their condition is const-evaluable). 35 existing stateless goldens regenerated to gain `const` on their public widget ctor; the lone exception is `m1_13_multiple_constructors`, where BOTH the canonical `const Counter({super.key, this.title = 'Counter'})` AND the named `const Counter.named({super.key}) : title = 'Named'` ctors gain `const` (literal-RHS rule), while the factory `Counter.fromInt` stays verbatim (factories are never const). Plain-class goldens (`m1_06`, `m4_08_effect_on_plain_class`, `m5_09_query_on_plain_class`, all `m6_02_*`) and existing-State-class goldens (`m1_07`, `m4_08_effect_on_state_class`, `m5_08_query_on_state_class`, `m6_05_environment_on_state_class`) are unaffected — no widget-ctor emission, no class split. `m1_12_passthrough_no_annotations` is unchanged because the rewriter writes the source verbatim back without invoking `_emitCtors`. The `goldenNames` const list in `test/integration/golden_helpers.dart` gained the four new entries appended after `m8_01_environment_extension_used`. `example/lib/counter.dart` and `example/lib/m6_09_environment_app.dart` were already hand-curated with `const CounterPage({super.key})` / `const CounterDisplay({super.key})` and remain byte-identical to what the generator now emits — no edits needed; running `build_runner` end-to-end on `example/source/` from this point forward produces clean output without manual fixups (the M8 raison d'être). No edits to `example/analysis_options.yaml` (no `prefer_const_constructors_in_immutables` suppression existed — each source widget has at least one mutable `late` / `@SolidState` field which trips the suppressed `must_be_immutable` lint first and short-circuits the const lint). No edits to `packages/solid_generator/test/golden/analysis_options.yaml` — its stale comment reference to `prefer_const_constructors_in_immutables` is left for a future cosmetic cleanup. Verification: `dart format --set-exit-if-changed .` reports 199 files / 0 changed; `dart analyze --fatal-infos` from the workspace root, `dart analyze packages/solid_generator/test/golden/outputs/`, and `dart analyze packages/solid_annotations` all report zero issues; `dart test packages/solid_generator/ -r github --fail-fast` passes 158 tests (M8-02's 150 plus four new goldens running through both the `golden` and `idempotency` suites); `flutter test example/ -r github` passes 9 widget/dispose tests; both `dart pub publish --dry-run` runs are clean modulo the expected mid-PR "checked-in files are modified in git" warning that resolves on commit.
